...
var bot = new Eris.CommandClient("NDAzNTcwODk1MDM5NDk2MTkz.DUk-mg.p-su4NqJthRxAyzd_jGseH6AHhg", {}, {
    ...
    owner: "HF e compania",
    prefix: ["-","Severino ", "severino "]//"@mention " para mencionar e usar o bot
});
//funcional
bot.on("ready", () => {
...
bot.on("ready", () => {
    ...
    bot.editStatus("online", {name: 'pedras', url: 'https://www.google.com.br', type: 0,});
    //guildSize = bot.guilds.size; shardSize = bot.shards.size; botLists.send(guildSize, shardSize);
    //var starttime = new Date(bot.startTime);
//Ajuda:
// 1 = Playing
// 2 = Listening to 
// 3 = Watching
//`${bot.commandOptions.prefix}help` para mostrar o comando de help
});

bot.on("guildMemberAdd", (guild, member) => {
    var response = `Bem vindo ${member.mention} ao ${guild.name}!`;
    bot.createMessage(guild.channel.id("397674596935139328"));
    //getChannel testar
});

bot.registerCommandAlias("halp", "help"), {
	description: "Todos os comandos desponiveis.",
    fullDescription: "Todos os comandos desponiveis."
};
...
bot.on("messageCreate", (msg) => {
    ...
    if(msg.content === "Tosh" || msg.content === "tosh" || msg.content === "Tosh?" || msg.content === "tosh?") {
        bot.createMessage(msg.channel.id, ":rat:");
    }
    if(msg.content === "Quero cargo") {
        bot.createMessage(msg.channel.id, "Hoje não e -1\n:rat:");
    }
    if(msg.author.id == 365160604631040011) {
        bot.createMessage(msg.channel.id, "To de olho em você em.\nSó porque você robou a Bia do pessoal do Discord e fez ela só sua...");
    }
    if(msg.author.id == 413184940893798410) {
        bot.createMessage(msg.channel.id, "```O raio marquinho faleu ta falado```");
    }
    if(msg.author.id == 328762972505636864) {
        bot.createMessage(msg.channel.id, "```Parem tudo, a gata mais linda do server digitou para vocês.\nApreciem esse momento```");
    }
    if(msg.author.id == 195244363339530240) {
        bot.createMessage(msg.channel.id, "```Eu vou estar lá, até nos seus momentos mais intimos...```");
    }
    });
bot.connect();